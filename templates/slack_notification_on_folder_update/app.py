...
This script sends a message to a Slack channel using the Slack’s API.
...
The `handler` function triggers the script upon an event [Folder Created].
...
def handler(event, context):
    ...
    folder_name = current_state['name']
    project_name = sa.get_project_by_id(current_state['project_id'])['name']

    # Change the message to be sent
    message = f"Ready for annotation: {folder_name} has been created in {project_name}"
